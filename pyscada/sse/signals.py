...
def _consume_historic(sender, instance, **kwargs):
    ...
    if type(instance) is Historic:
        logger.info(
            f"consume_historic {type(instance).__name__}.{getattr(instance, 'id', 'new')}-session-{getattr(instance, 'session_key')}-view-{getattr(getattr(instance, 'view'), 'id')}"
        )
        try:
            instance.send_message({"historic": "saved"}, async_publish=True)
            if not instance.done and (
                instance.variables.count()
                or instance.status_variables.count()
                or instance.variable_properties.count()
            ):
